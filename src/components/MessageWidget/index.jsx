...
function Index({ pusher }) {
  ...
  const options = {
    ...
    authEndpoint: PublicBaseUrl + "api/broadcasting/auth",
  ...
  let newMessageHandler = (n, id) => {
    if (auth()) {
      if (n.message.seller_id === id) {
        ...
        axios
          .get(
            `${PublicBaseUrl}api/user/message-with-seller?token=${
              auth().access_token
            }`
          )
          .then((res) => {
  ...
  const activeSellerHandler = (value, vendorId) => {
    ...
    if (auth()) {
      axios
        .get(
          `${PublicBaseUrl}api/user/load-active-seller-message/${parseInt(
            value
  ...
  const send = (productId) => {
    if (auth()) {
      if (selectedSellerId) {
        axios
          .post(
            `${PublicBaseUrl}api/user/send-message-to-seller?token=${
              auth().access_token
            }`,
            {
  ...
  useEffect(() => {
    if (getSlug) {
      if (!isSeller && !product) {
        axios.get(`${PublicBaseUrl}api/product/${getSlug}`).then((res) => {
          if (res) {
            if (res.data) {
              if (
                        ...
                        <div className={`w-[44px] h-[44px]  relative `}>
                          <div className="w-full h-full rounded-full overflow-hidden bg-[#FAF9FA] relative shadow-lg">
                            <Image
                              layout="fill"
                              objectFit="contain"
                              src={PublicBaseUrl + item.shop_logo}
                              alt="seller"
                            />
                          </div>
                        ...
                        <div className={`flex space-x-2.5 items-center`}>
                          ...
                          >
                            <Image
                              layout="fill"
                              objectFit="contain"
                              src={`${PublicBaseUrl + product.thumb_image}`}
                              alt="seller"
                            />
                          </div>
